<div>
    <div class="chatlist_header py-2 pl-4 pr-2 flex justify-between items-center">
        <h2 class="text-2xl font-bold">Chat</h2>
        <img src="{{ asset('img/profile-1.jpeg') }}" class="w-14 h-14 rounded-full" alt="profile">
    </div>


    <div class="chatlist_body">
        <div class="row m-0 pl-4 pr-2 bg-slate-400">
            <div class="col-3 m-0 p-0 rounded-full overflow-hidden">
                <img src="{{ asset('img/profile-2.jpg') }}" class="w-14 h-14 object-cover object-center w-full h-full" alt="profile">
            </div>
            <div class="col-7">123</div>
            <div class="col-2">123</div>
        </div>
    </div>
</div>
