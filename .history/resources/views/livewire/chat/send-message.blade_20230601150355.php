<div class="row col-12 mx-auto py-2 px-4">

    <form wire:submit.prevent="send_message" action="">
        <div class="col-9 m-0 p-0 my-auto">
            <input type="text" wire:model="body" name="body" id="body" placeholder="Type your message here ..." class="w-full bg-gray-300 border-0 rounded-xl">
        </div>

        <div class="col-3 text-center my-auto cursor-pointer">
            <button type="submit" class="w-fit flex px-4 py-2 text-lg bg-blue-400 rounded-full mx-auto flex space-x-2">
                <span><i class="fa-regular fa-paper-plane"></i></span>
                <span>Send</span>
            </button>
        </div>
    </form>

</div>
