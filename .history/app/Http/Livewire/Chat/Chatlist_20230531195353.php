...
class Chatlist extends Component
{
    ...
    private function format_conversations(Array $conversations)
    {
        return array_map(function ($conversation) {
            ...
            $conversation['last_converse'] = Carbon::parse($last_message['created_at'])->diffForHumans();

            // $now = Carbon::now('UTC')->format('Y m d h m s');
            // dd($now);
            // $diffInMinutes = Carbon::parse($last_message['created_at'])->shortAbsoluteDiffForHumans();
            // $conversation['123'] = $diffInMinutes;
            // $unit = '';
